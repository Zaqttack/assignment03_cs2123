...
int main( int argc, char *argv[] )
{
    ...
    for( i=0; i<6; i++){
        ...
        addNewProcessToSchedule( ps, temp, getRuntime(temp), getPriority(temp) );
    }

    //Simulate time steps until the schedule is finished (i.e. all work finished for all processes)
    while( isScheduleUnfinished( ps ) ){
        name = simulateNextTimeStep( ps );
        if( name!= NULL )
            addNewProcessToSchedule( ps, name, getRuntime(name), getPriority(name) );
    }
    freeSchedule( ps );
